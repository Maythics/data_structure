...
/// 模仿delete删除按键，但是是把current也删除，currentPos 元素自己。若当前位置为空则不进行操作；若链表仅有一个元素则删除此元素成为空链表；若链表有超过一个元素且当前位置不为最后一个，则删除后一个元素.
template <typename T>
void SingleLinkedList<T>::remove()
{
    ...
    ///否则，把自己删除，先得找到前一个Node
    Node*p = head;
    while(p->next != currentPos)
    {
        p = p->next;
    }
    ///此时，p就是currentPos的前一个节点了

    ///若恰好currentPos就是最后的一个，则删除后变成p（相当于仍是最后一个）
